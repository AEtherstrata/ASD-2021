...
{
	arrayList* newElement = (arrayList*)malloc(sizeof(arrayList)); // Allocate a new list node
	
	int* result = (int*)calloc(len, sizeof(int));    // Allocate space for a mirrored origin array
...
{
	if (a == NULL) {
		printf("max_value(): The specified array is empty!. Exiting...\n");
		exit(1);
	}
